...
var Agenda = module.exports = function Agenda(config) {
  ...
  if (!config.model) {
    ...
    throw new Error('friendly default setup is not ready yet!');
  }

  // Just a little duck-typing for a better user experience:
  if (typeof config.model !== 'object' || !config.model.update || !config.model.create || !config.model.upsert) {
    throw new Error('Provided `model` is not valid (note that it must have an `upsert` method)');
  }
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  // (i.e. "update if it exists, otherwise create")
    ...
    ////////////////////////////////////////////////////////////////////////////////////////
    // TODO:
    // Implement the following logic somehow (~mike)
  ...
  function processDbResult(err, res){
    ...
    try {
      ...
      job.attrs._id = res.id;
      job.attrs.nextRunAt = res.nextRunAt;

      // Coerce database result set into a single record, if relevant:
