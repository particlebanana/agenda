...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if(!props.nextRunAt) delete props.nextRunAt;
  ...
  if (id) {
    ...
    this._model.update({ id: id }, props, processDbResult);
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  .exec(function(err, result) {
    ...
    result.lockedAt = now;
    _self._model.update({ id: result.id }, result).exec(findJobsResultWrapper(_self, cb));
  });
  ...
  //   function(err, _rs) {
  //     console.log(err, _rs);
  //     findJobsResultWrapper(_self, cb)
  //   }
  // );
...
function processJobs(extraJob) {
  ...
  function processJobResult(err, job) {
    ...
    // Remove the job definition if it doesn't need to be run again
    if(!job.attrs.nextRunAt) {
      self._model.destroy({ id: job.attrs.id }).exec(function() {
        jobProcessing();
      });
    }
    else {
